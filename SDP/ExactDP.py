...
def sdp_dam(max_iter=100, tol=0.1, Print_Iterations=True, debug=False):
                ...
                for release in RELEASES:
                    ...
                    for qr_idx1 in range(M):
                        ...
                        for qr_idx2 in range(M):
                            ...
                            row_sum += prob * (reward + penalty + beta * future)
    ...
    # Save policy arrays once after convergence
    os.makedirs("policy_npy", exist_ok=True)
    for m, P in policy.items():
        np.save(f"policy_npy/policy_{m}.npy", P)
